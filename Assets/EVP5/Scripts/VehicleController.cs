...
namespace EVP
{
    ...
    public class VehicleController : NetworkBehaviour
    {
        ...
        [Header("Center of Mass")] public CenterOfMassMode centerOfMassMode = CenterOfMassMode.Parametric;
        [Range(0.1f, 0.9f)] public float centerOfMassPosition = 0.5f;
        [Range(-1.0f, 1.0f)] public float centerOfMassHeightOffset = 0.0f;
        [FormerlySerializedAs("centerOfMass")] public Transform centerOfMassTransform;

        [Header("Vehicle Setup")] [FormerlySerializedAs("maxSpeed")]
        public float maxSpeedForward = 27.78f;

        public float maxSpeedReverse = 12.0f;
        [Range(0, 3)] public float tireFriction = 1.0f;
        [Range(0, 1)] public float rollingResistance = 0.05f;
        [Range(0, 1)] public float antiRoll = 0.2f;
        [Range(0, 89)] public float maxSteerAngle = 35.0f;
        [Range(0, 4)] public float aeroDrag = 0.0f;
        [Range(0, 2)] public float aeroDownforce = 1.0f;

        [Header("Vehicle Balance")] [Range(0, 1)]
        public float driveBalance = 0.5f;

        [Range(0, 1)] public float brakeBalance = 0.5f;
        [Range(0.3f, 0.7f)] public float tireFrictionBalance = 0.5f;
        [Range(0, 1)] public float aeroBalance = 0.5f;
        [Range(0, 1)] public float handlingBias = 0.5f;
        ...
        [Range(0.0001f, 0.9999f)] public float forceCurveShape = 0.5f;
        public float maxDriveSlip = 4.0f;
        public float driveForceToMaxSlip = 1000.0f;

        [Header("Brakes")] public float maxBrakeForce = 3000.0f;
        ...
        [Range(0, 1)] public float maxBrakeRatio = 0.5f;

        public BrakeMode handbrakeMode = BrakeMode.Slip;
        public float maxHandbrakeSlip = 10.0f;
        [Range(0, 1)] public float maxHandbrakeRatio = 1.0f;

        [Header("Driving Aids")] [FormerlySerializedAs("tcEnabled")]
        public bool tractionControl = false;

        [Range(0, 1)] [FormerlySerializedAs("tcRatio")]
        public float tractionControlRatio = 1.0f;

        [FormerlySerializedAs("absEnabled")] public bool brakeAssist = false;

        [Range(0, 1)] [FormerlySerializedAs("absRatio")]
        public float brakeAssistRatio = 1.0f;

        [FormerlySerializedAs("espEnabled")] public bool steeringLimit = false;

        [Range(0, 1)] [FormerlySerializedAs("espRatio")]
        public float steeringLimitRatio = 0.5f;

        public bool steeringAssist = true;
        [Range(0, 1)] public float steeringAssistRatio = 0.5f;

        // Vehicle controls

        [Range(-1, 1)] public float steerInput = 0.0f;
        [Range(-1, 1)] public float throttleInput = 0.0f;
        [Range(0, 1)] public float brakeInput = 0.0f;
        [Range(0, 1)] public float handbrakeInput = 0.0f;
        ...
        public float
            impactThreeshold =
                0.6f; // 0.0 - 1.0. The DotNormal of the impact is calculated. Less than this value means drag, more means impact.
        ...
        [NonSerialized]
        public bool
            computeExtendedTireData =
                false; // Components using extended tire data (tire marks, smoke, particles, audio) set this to True
        ...
        {
            get { return m_sumImpactPosition; }
        }

        public Vector3 localImpactVelocity
        {
            get { return m_sumImpactVelocity; }
        }

        public bool isHardImpact
        {
            get { return m_sumImpactHardness >= 0; }
        }
        ...
        {
            get { return m_localDragPosition; }
        }

        public Vector3 localDragVelocity
        {
            get { return m_localDragVelocity; }
        }

        public bool isHardDrag
        {
            get { return m_localDragHardness >= 0; }
        }
        ...
        // Specific positions in the vehicle

        public Vector3 frontAxlePosition
        {
            get
            {
                return transform.TransformPoint(new Vector3(0.0f, m_vehicleFrame.baseHeight,
                    m_vehicleFrame.frontPosition));
            }
        }

        public Vector3 rearAxlePosition
        {
            get
            {
                return transform.TransformPoint(new Vector3(0.0f, m_vehicleFrame.baseHeight,
                    m_vehicleFrame.rearPosition));
            }
        }
        ...
        void OnEnable()
        {
            ...
            {
                Debug.LogWarning(
                    "The wheels property is empty. You must configure wheels and WheelColliders first. Component is disabled.");
                enabled = false;
                return;
            }
            ...
            for (int i = 0; i < m_wheelData.Length; i++)
            {
                ...
                {
                    Debug.LogError("A WheelCollider is missing in the list of wheels for this vehicle: " +
                                   gameObject.name);
                    enabled = false;
                    return;
                }

                if (w.caliperTransform != null && w.wheelTransform != null &&
                    w.caliperTransform.IsChildOf(w.wheelTransform))
                {
                    Debug.LogWarning(this.ToString() + ": caliper (" + w.caliperTransform.name +
                                     ") should not be child of wheel (" + w.wheelTransform.name + ").\n"
                                     + "Visual issues will surely appear. Either make wheel child of caliper, or put both at the same level (siblings).");
                }

                wd.isWheelChildOfCaliper = w.caliperTransform != null && w.wheelTransform != null &&
                                           w.wheelTransform.IsChildOf(w.caliperTransform);
            ...
            m_lastImpactedMaterial =
                new PhysicMaterial(); // A new reference to ensure cache missmatch at the first query
        }

        public override void OnStartLocalPlayer()
        {
            FindObjectOfType<VehicleCameraController>().target = transform;
        }
        ...
        void Update()
        {
            ...
            else if (wheelUpdateRate == UpdateRate.OnUpdate || wheelPositionMode == PositionMode.Accurate)
            {
        ...
        void ComputeSteerAngle()
        {
            ...
            if (steeringAssist)
                assistedSteerAngle = m_speedAngle * steeringAssistRatio * speedFactor *
                                     Mathf.InverseLerp(2.0f, 3.0f, Mathf.Abs(m_speedAngle));
        ...
        float FixSteerAngle(WheelData wd, float inputSteerAngle)
        {
            ...
            return Vector3.Angle(m_transform.forward, rbWheelForward) *
                   Mathf.Sign(Vector3.Dot(m_transform.right, rbWheelForward));
        }
        ...
        void UpdateSuspension(WheelData wd)
        {
            ...
            if (wd.grounded)
            {
                wd.suspensionCompression = 1.0f -
                                           (-wd.transform.InverseTransformPoint(wd.hit.point).y - wd.collider.radius) /
                                           wd.collider.suspensionDistance;
        ...
        void UpdateLocalFrame(WheelData wd)
        {
            ...
            float estimatedSprungMass =
                Mathf.Clamp(wd.hit.force / -Physics.gravity.y, 0.0f, wd.collider.sprungMass) * 0.5f;
        ...
        {
            if (wd.grounded)
                UpdateGroundMaterialCached(wd.hit.collider.sharedMaterial, ref wd.lastPhysicMaterial,
                    ref wd.groundMaterial);
        }
        ...
        void ComputeTireForces(WheelData wd)
        {
            ...
            if (wd.wheel.brake && wd.wheel.handbrake)
            {
                ...
                if (handbrakeInput >= brakeInput)
                    ComputeBrakeValues(wd, handbrakeMode, maxHandbrakeSlip, maxHandbrakeRatio, out wheelBrakeSlip,
                        out wheelBrakeRatio);
                else
                    ComputeBrakeValues(wd, brakeMode, maxBrakeSlip, maxBrakeRatio, out wheelBrakeSlip,
                        out wheelBrakeRatio);
            }
            ...
            float combinedInput = -rollingResistance + absThrottleInput * (1.0f + rollingResistance) -
                                  wheelBrakeInput * (1.0f - rollingResistance);
            ...
            if (wd.grounded)
            {
                ...
                if (wd.isBraking)
                {
                    float wheelMaxBrakeSlip =
                        Mathf.Max(Mathf.Abs(wd.localVelocity.y * wheelBrakeRatio), wheelBrakeSlip);
                    minSlipY = Mathf.Clamp(Mathf.Abs(demandedForce * wd.tireSlip.x) / forceMagnitude, 0.0f,
                        wheelMaxBrakeSlip);
                }
                else
                {
        ...
        public Vector3 GetSidewaysForceAppPoint(WheelData wd, Vector3 contactPoint)
        {
            ...
            if (wd.wheel.steer && wd.steerAngle != 0.0f && Mathf.Sign(wd.steerAngle) != Mathf.Sign(wd.tireSlip.x))
                sidewaysForcePoint += wd.transform.forward *
                                      (m_vehicleFrame.frontPosition - m_vehicleFrame.rearPosition) *
                                      (handlingBias - 0.5f);
        ...
        void ComputeBrakeValues(WheelData wd, BrakeMode mode, float maxSlip, float maxRatio, out float brakeSlip,
            out float brakeRatio)
        {
        ...
        void UpdateTransform(WheelData wd, float deltaTime)
        {
            if (wd.wheel.wheelTransform != null || wd.wheel.caliperTransform != null)
            {
                ...
                {
                    elongation = wd.collider.suspensionDistance * (1.0f - wd.suspensionCompression) +
                                 wd.collider.radius * 0.05f;
                    wd.visualHit.point = wd.hit.point;
                    wd.visualHit.normal = wd.hit.normal;
                    wd.visualHitCollider = wd.hit.collider;
                    wd.visualGrounded = wd.grounded;
                }
                else
                {
                    wd.visualGrounded = Physics.Raycast(wd.origin, -wd.transform.up, out wd.visualHit,
                        (wd.collider.suspensionDistance + wd.collider.radius), Physics.DefaultRaycastLayers,
                        QueryTriggerInteraction.Ignore);

                    if (wd.visualGrounded)
                    {
                        // If these layers are intended to ignore collisions then just use the actual WheelHit information

                        if (Physics.GetIgnoreLayerCollision(wd.collider.gameObject.layer,
                                wd.visualHit.collider.gameObject.layer))
                        {
                            elongation = wd.collider.suspensionDistance * (1.0f - wd.suspensionCompression) +
                                         wd.collider.radius * 0.05f;
                            wd.visualHit.point = wd.hit.point;
                        ...
                        {
                            elongation = wd.visualHit.distance - wd.collider.radius * 0.95f;
                            wd.visualHitCollider = wd.visualHit.collider;
                        }
                    }
                    else
                    {
                ...
                if (wd.wheel.caliperTransform != null)
                {
                    ...
                    wd.wheel.caliperTransform.rotation =
                        wd.transform.rotation * Quaternion.Euler(0.0f, wd.steerAngle, 0.0f);
                }
                ...
                if (wd.wheel.wheelTransform != null)
                {
                    ...
                    if (wd.isWheelChildOfCaliper)
                    {
                        ...
                        wd.wheel.wheelTransform.localRotation =
                            Quaternion.Euler(wd.angularPosition * Mathf.Rad2Deg, 0.0f, 0.0f);
                    }
                    else
                    {
                        ...
                        wd.wheel.wheelTransform.rotation = wd.transform.rotation *
                                                           Quaternion.Euler(wd.angularPosition * Mathf.Rad2Deg,
                                                               wd.steerAngle, 0.0f);
                    }
                }
            }
        ...
        void UpdateGroundMaterialCached(PhysicMaterial colliderMaterial, ref PhysicMaterial cachedMaterial,
        ...
        public float SphereRaycastOthers(Vector3 origin, Vector3 direction, float radius, float maxDistance,
            ...
            foreach (Wheel w in wheels)
            {
                if (w.wheelCollider != null)
                {
                    middlePoint += transform
                        .InverseTransformPoint(w.wheelCollider.transform.TransformPoint(w.wheelCollider.center)).z;
                    middleCount++;
                }
            }
                ...
                if (w.wheelCollider != null)
                {
                    Vector3 localPos =
                        transform.InverseTransformPoint(
                            w.wheelCollider.transform.TransformPoint(w.wheelCollider.center));
        ...
        void ProcessContacts(Collision col, bool isCollisionEnter)
        {
            ...
            foreach (ContactPoint contact in col.contacts)
            {
                ...
                int hardness = 0;
                UpdateGroundMaterialCached(collider.sharedMaterial, ref m_lastImpactedMaterial,
                    ref m_impactedGroundMaterial);
                ...
                if (dragRatio < -impactThreeshold ||
                    isCollisionEnter && col.relativeVelocity.sqrMagnitude > sqrImpactSpeed)
                {
                    // Impact
            ...
            if (dragCount > 0)
            {
                ...
                UpdateDragState(m_transform.InverseTransformPoint(dragPosition),
                    m_transform.InverseTransformDirection(dragVelocity), dragHardness);
            }
        }
        ...
        void HandleImpacts()
        {
            ...
            if (Time.time - m_lastImpactTime >= impactInterval && m_sumImpactCount > 0)
            {
                ...
                if (showCollisionGizmos && localImpactVelocity.sqrMagnitude > 0.001f)
                    Debug.DrawLine(transform.TransformPoint(localImpactPosition),
                        transform.TransformPoint(localImpactPosition) +
                        CommonTools.Lin2Log(transform.TransformDirection(localImpactVelocity)), Color.red, 0.2f, false);
                ...
                m_lastImpactTime = Time.time +
                                   impactInterval *
                                   UnityEngine.Random.Range(-impactIntervalRandom,
                                       impactIntervalRandom); // Add a random variation for avoiding regularities
            }
        }
        ...
        void UpdateDragState(Vector3 dragPosition, Vector3 dragVelocity, int dragHardness)
        {
            ...
            if (showCollisionGizmos && localDragVelocity.sqrMagnitude > 0.001f)
                Debug.DrawLine(transform.TransformPoint(localDragPosition),
                    transform.TransformPoint(localDragPosition) +
                    CommonTools.Lin2Log(transform.TransformDirection(localDragVelocity)), Color.cyan, 0.05f, false);
        }
        ...
        static void AdjustColliderToWheelMesh(WheelCollider wheelCollider, Transform wheelTransform)
        {
            ...
            wheelCollider.transform.position =
                wheelTransform.position + wheelTransform.up * wheelCollider.suspensionDistance * 0.5f;
            ...
            {
                Debug.LogWarning(wheelTransform.gameObject.name +
                                 ": Couldn't calculate radius. There are no meshes in the Wheel transform or its children");
                return;
            }
            ...
            if (Mathf.Abs(bounds.extents.y - bounds.extents.z) > 0.01f)
                Debug.LogWarning(wheelTransform.gameObject.name +
                                 ": The Wheel mesh might not be a correct wheel. The calculated radius is different along forward and vertical axis.");
        ...
        public void OnDrawGizmos()
        {
            ...
            Vector3 middle = transform.TransformPoint(new Vector3(0.0f, frame.baseHeight,
                (frame.frontPosition + frame.rearPosition) * 0.5f));
            ...
            UnityEditor.Handles.DrawLine(front - transform.right * frame.frontWidth,
                front + transform.right * frame.frontWidth);
            UnityEditor.Handles.DrawLine(rear - transform.right * frame.rearWidth,
                rear + transform.right * frame.rearWidth);
            ...
            UnityEditor.Handles.DrawLine(drivePos - featureDir - driveForwardDir,
                drivePos - featureDir + driveForwardDir);
            UnityEditor.Handles.DrawLine(drivePos + featureDir - driveForwardDir,
                drivePos + featureDir + driveForwardDir);
